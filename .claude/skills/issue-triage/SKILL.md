...
- **Воркер** — `claude -p --dangerously-skip-permissions` в worktree `../<repo>.worktrees/agents-<slug>`
  на ветке `agents/<slug>`. Headless: не зависает на вопросах, чисто завершается → освобождает слот.
- **Пул** — `run.sh` держит до `CONCURRENCY` (дефолт 5) живых воркеров; следующий стартует, когда
  предыдущий уронил sentinel-файл. tmux-окна остаются открытыми (`remain-on-exit on`) + лог на воркер.
