...
//   run orchestrate      — один тик оркестратора, видно каждый его шаг
...
const USAGE = `Использование: run <роль|orchestrate> [аргументы скилла] [--worktree <имя>] [--here]
  ...
  run orchestrate                один тик оркестратора — видно каждый его шаг
  run orchestrate --print        то же, но как у демона: только итоговая строка
...
function passthrough(args: string[], label: string): Promise<number> {
    ...
    return new Promise(resolve => {
        // stdin тоже отдаём: запуск руками — интерактивный, в него можно вмешаться и договорить.
        const child = spawn("claude", args, { cwd: REPO_ROOT, stdio: "inherit" });
...
async function main(argv: string[]): Promise<number> {
    ...
    if (target === "orchestrate") {
        ...
        // Демон гоняет тик через -p и видит только итог. Здесь наоборот: интерактивно,
        // чтобы было видно каждый вызов инструмента и рассуждения — ради этого руками и запускают.
        // `--print` возвращает поведение демона, если нужно проверить именно его.
        const quiet = argv.includes("--print");
        return passthrough(
            [
                ...(quiet ? ["-p"] : []),
                "/orchestrate",
