...
- **Контролы TUIDom — «вещь в себе»** — TUIDom не импортирует Theme и app-концепции; контрол несёт plain color-props с theme-less дефолтами, маппинг темы — на стороне приложения (`applyXxxTheme`/`applyStyles`). App-UI собирается на слое Workbench по модели Service ↔ Component. Детали → [docs/arch/Workbench.md](docs/arch/Workbench.md).
