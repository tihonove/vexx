...
- **Зависимости** — внешние зависимости брать **можно**: не пиши своё там, где задача фундаментальная (не наша предметная) и есть сфокусированная leaf-библиотека без транзитивного хвоста — так уже живём (`vscode-textmate`, `vscode-oniguruma`, `jsonc-parser`, `chokidar`, `yauzl`). Запрет — только на фреймворки, претендующие на рендер-цикл и модель элементов (Ink/Blessed/React/Vue). Критерии → [GOAL.md](GOAL.md).
