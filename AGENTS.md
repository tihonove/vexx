...
- **Цвета** — все цвета UI берутся только из активной темы через `theme.getColor`/`getRequiredColor`; никаких литералов и инлайн-фоллбэков. Новый цвет — определение `{ defaults: { dark, light }, description }` в группе своей области в `src/Theme/colors/`. Детали → [docs/arch/Theme.md](docs/arch/Theme.md).
