...
describe("TerminalRenderer", () => {
    ...
    describe("diffing: only changed cells are emitted", () => {
        it("produces no output when grids are identical", () => {
            const cap = createCapture();
            const renderer = new TerminalRenderer(cap.writer);
            const a = new Grid(3, 2);
            const b = new Grid(3, 2);
        ...
        it("emits only the changed cell", () => {
            ...
            const current = new Grid(3, 1);
            const previous = new Grid(3, 1);

            // Change only the middle cell
            current.setCell(1, 0, "X");

            renderer.render(current, previous);
            // Should see: cursor-to(1,0) + reset + "X" + trailing reset
            expect(humanize(cap.output)).toMatchInlineSnapshot(`"ESC[1;2HESC[0mXESC[0m"`);
        });
        ...
        it("does not re-emit on second render if nothing changed", () => {
            ...
            const current = new Grid(2, 1);
            const previous = new Grid(2, 1);
            current.setCell(0, 0, "A");
    ...
    describe("cursor positioning", () => {
        it("skips CUP for consecutive cells on the same row", () => {
            ...
            const current = new Grid(4, 1);
            const previous = new Grid(4, 1);
            current.setCell(0, 0, "A");
            current.setCell(1, 0, "B");
            current.setCell(2, 0, "C");

            renderer.render(current, previous);
            const h = humanize(cap.output);

            // Only one CUP at the beginning (1;1H), then A B C without extra CUP
            expect(h).toMatchInlineSnapshot(`"ESC[1;1HESC[0mABCESC[0m"`);
        });
        ...
        it("emits CUP when there is a gap between changed cells", () => {
            ...
            const current = new Grid(5, 1);
            const previous = new Grid(5, 1);
            current.setCell(0, 0, "A");
            // cell 1 unchanged
            current.setCell(2, 0, "C");
            ...
            expect(h).toMatchInlineSnapshot(`"ESC[1;1HESC[0mAESC[1;3HCESC[0m"`);
        });
        ...
        it("emits CUP when row changes", () => {
            ...
            const current = new Grid(2, 2);
            const previous = new Grid(2, 2);
            current.setCell(0, 0, "A");
            current.setCell(0, 1, "B");

            renderer.render(current, previous);
            const h = humanize(cap.output);

            // CUP to row 1 col 1, "A", CUP to row 2 col 1, "B"
            expect(h).toMatchInlineSnapshot(`"ESC[1;1HESC[0mAESC[2;1HBESC[0m"`);
        });
    });
    ...
    describe("TrueColor SGR sequences", () => {
        it("emits foreground color as 38;2;R;G;B", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "R", packRgb(255, 0, 0));
        ...
        it("emits background color as 48;2;R;G;B", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, " ", DEFAULT_COLOR, packRgb(0, 128, 255));
        ...
        it("emits both fg and bg when both are set", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "X", packRgb(255, 255, 0), packRgb(0, 0, 128));
    ...
    describe("state machine avoids redundant SGR", () => {
        it("does not re-emit fg color for consecutive cells with same fg", () => {
            ...
            const current = new Grid(3, 1);
            const previous = new Grid(3, 1);
            current.setCell(0, 0, "A", red);
            current.setCell(1, 0, "B", red);
            current.setCell(2, 0, "C", red);
        ...
        it("emits new fg only when color changes mid-row", () => {
            ...
            const current = new Grid(3, 1);
            const previous = new Grid(3, 1);
            current.setCell(0, 0, "R", red);
            current.setCell(1, 0, "G", green);
            current.setCell(2, 0, "R", red);
        ...
        it("resets to default fg via SGR 39", () => {
            ...
            const previous = new Grid(2, 1);
            current.setCell(0, 0, "R", red);
            current.setCell(1, 0, "D"); // DEFAULT_COLOR fg

            renderer.render(current, previous);
            const h = humanize(cap.output);

            // After red cell, default-fg cell should emit ESC[39m
            expect(h).toMatchInlineSnapshot(`"ESC[1;1HESC[0mESC[38;2;255;0;0mRESC[39mDESC[0m"`);
        });
        ...
        it("resets to default bg via SGR 49", () => {
            ...
            const current = new Grid(2, 1);
            const previous = new Grid(2, 1);
            current.setCell(0, 0, "B", DEFAULT_COLOR, blue);
            current.setCell(1, 0, "D"); // DEFAULT_COLOR bg
    ...
    describe("style flags produce correct SGR codes", () => {
        it("Bold → SGR 1", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "B", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Bold);
        ...
        it("Dim → SGR 2", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "D", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Dim);
        ...
        it("Italic → SGR 3", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "I", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Italic);
        ...
        it("Underline → SGR 4", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "U", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Underline);
        ...
        it("Undercurl → SGR 4:3", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "~", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Undercurl);
        ...
        it("Inverse → SGR 7", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "V", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Inverse);
        ...
        it("Strikethrough → SGR 9", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "S", DEFAULT_COLOR, DEFAULT_COLOR, StyleFlags.Strikethrough);
    ...
    describe("style change triggers full SGR reset", () => {
        it("resets and re-applies when style changes mid-row", () => {
            ...
            const current = new Grid(2, 1);
            const previous = new Grid(2, 1);
            current.setCell(0, 0, "B", white, DEFAULT_COLOR, StyleFlags.Bold);
            current.setCell(1, 0, "I", white, DEFAULT_COLOR, StyleFlags.Italic);
    ...
    describe("full scene snapshot", () => {
        it("renders a 4×2 grid with mixed colors and styles", () => {
            ...
            const current = new Grid(4, 2);
            const previous = new Grid(4, 2);

            // Row 0: "Hi" in red bold, then "!!" in green
            current.setCell(0, 0, "H", red, DEFAULT_COLOR, StyleFlags.Bold);
            current.setCell(1, 0, "i", red, DEFAULT_COLOR, StyleFlags.Bold);
            current.setCell(2, 0, "!", green);
            current.setCell(3, 0, "!", green);

            // Row 1: "  " with blue bg, then "OK" default
            current.setCell(0, 1, " ", DEFAULT_COLOR, blue);
            current.setCell(1, 1, " ", DEFAULT_COLOR, blue);
            current.setCell(2, 1, "O");
            current.setCell(3, 1, "K");

            renderer.render(current, previous);

            expect(humanize(cap.output)).toMatchInlineSnapshot(
                `"ESC[1;1HESC[0mESC[1mESC[38;2;255;0;0mHiESC[0mESC[38;2;0;200;0m!!ESC[2;1HESC[0mESC[48;2;0;0;255m  ESC[49mOKESC[0m"`,
            );
        });
    });
    ...
    describe("previousGrid update", () => {
        it("updates previousGrid cells in-place after render", () => {
            ...
            const current = new Grid(2, 1);
            const previous = new Grid(2, 1);
            current.setCell(0, 0, "A", packRgb(1, 2, 3));
            current.setCell(1, 0, "B", packRgb(4, 5, 6));

            renderer.render(current, previous);

            // After render, previous should mirror current
            expect(previous.getCell(0, 0).char).toBe("A");
            expect(previous.getCell(0, 0).fg).toBe(packRgb(1, 2, 3));
            expect(previous.getCell(1, 0).char).toBe("B");
            expect(previous.getCell(1, 0).fg).toBe(packRgb(4, 5, 6));
        });
    });
    ...
    describe("trailing SGR reset", () => {
        it("appends ESC[0m when last emitted cell has non-default attributes", () => {
            ...
            const current = new Grid(1, 1);
            const previous = new Grid(1, 1);
            current.setCell(0, 0, "X", packRgb(255, 0, 0));
        ...
        it("does not append trailing reset when all cells are default-colored", () => {
            ...
            expect(h).toMatchInlineSnapshot(`"ESC[1;1HESC[0mXESC[0m"`);
        });
    });
});
