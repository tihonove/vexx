...
function makeEditor(lineContent: string, character: number): { editor: EditorController; fake: FakeEditor } {
...
function setup(items: readonly ICoreCompletionItem[], lineContent = "ind", character = 3) {
    const { editor, fake } = makeEditor(lineContent, character);
    ...
    return { controller, body, testApp, fake, source, onExecuteCommand };
}
...
describe("CompletionController", () => {
    it("нет источника → trigger no-op (попап скрыт)", async () => {
        const group = { getActiveEditor: () => makeEditor("", 0).editor, completionSource: undefined } as unknown as EditorGroupController;
        const controller = new CompletionController(group);
        const body = new BodyElement();
