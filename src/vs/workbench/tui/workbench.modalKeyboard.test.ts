...
describe("AppController — global keybindings are suppressed while an overlay owns the keyboard", () => {
    ...
    it("quickpick swallows Ctrl+B: sidebar does not toggle, focus stays, overlay stays up", () => {
        ...
        const before = h.controller.workbenchLayout.getLeftPanelVisible();
        ...
        expect(h.controller.workbenchLayout.getLeftPanelVisible()).toBe(before); // global did NOT fire
    ...
    it("focus-scoped editing works in the quickpick while workbench shortcuts stay suppressed", () => {
        ...
        h.testApp.sendKey("Ctrl+B");
        h.testApp.render();
        expect(h.controller.workbenchLayout.getLeftPanelVisible()).toBe(before);
    ...
    it("a modal confirm dialog swallows Ctrl+B", () => {
        h.controller.showConfirmSaveDialog("alpha.txt", {
            onSave: () => {},
            onDontSave: () => {},
            onCancel: () => {},
        });
