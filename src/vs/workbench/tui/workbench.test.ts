...
import type { LogEntry } from "../../platform/log/common/log.ts";
...
import type { EditorTabStripElement } from "./parts/editor/editorTabStripElement.ts";
import type { QuickPickElement } from "../../platform/quickinput/tui/quickPickElement.ts";
import type { StatusBarElement } from "./parts/statusbar/statusBarElement.ts";

import type { AppController } from "./workbench.ts";
...
describe("AppController integration", () => {
    it("creates UI tree with menubar and editor", () => {
        ...
        h.controller.openFile("/tmp/test-tree.txt");
    ...
    it("focuses editor via focusEditor()", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/test-focus.txt");
    ...
    it("typing inserts text into editor", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/test-typing.txt");
        h.controller.focusEditor();
    ...
    it("Ctrl+Tab switches to next editor tab", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/tab-a.txt");
        h.controller.openFile("/tmp/tab-b.txt");
        h.controller.focusEditor();
    ...
    it("statusbar shows the cursor position after openFile", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/test-app-statusbar.txt");
    ...
    it("statusbar updates the cursor position live after typing", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/test-app-modified.txt");
        h.controller.focusEditor();
...
describe("AppController — chords", () => {
    ...
    it("Ctrl+K then S runs the chord-bound save without leaking 's' into the editor", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-save.txt");
        h.controller.focusEditor();
        const executeSpy = vi.spyOn(h.commands, "execute");
    ...
    it("a continuation key that matches no command is swallowed, not typed", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-swallow.txt");
        h.controller.focusEditor();
        const executeSpy = vi.spyOn(h.commands, "execute");
    ...
    it("reports the unmatched combination in the status bar when a chord is not completed", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-notfound.txt");
        h.controller.focusEditor();
    ...
    it("the 'not a command' message auto-clears after the timeout", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-notfound-clear.txt");
        h.controller.focusEditor();
    ...
    it("Ctrl+K then Ctrl+S breaks the chord and is consumed (no save, no leak)", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-ctrls.txt");
        h.controller.focusEditor();
        const executeSpy = vi.spyOn(h.commands, "execute");
    ...
    it("resolves the chord through the full Kitty key lifecycle (down/up incl. release)", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-kitty.txt");
        h.controller.focusEditor();
        const executeSpy = vi.spyOn(h.commands, "execute");
    ...
    it("a new keypress clears the pending 'not a command' timer before it fires", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-notfound-reset.txt");
        h.controller.focusEditor();
    ...
    it("a focus change cancels an in-progress chord and clears the waiting hint", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/chord-focus-cancel.txt");
        h.controller.focusEditor();
...
describe("AppController — Quick Open", () => {
    ...
    it("Escape after Ctrl+P returns focus to editor", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/focus-restore.txt");
        h.controller.focusEditor();
        const editorElement = h.testApp.querySelector("EditorElement");
...
describe("AppController — completion wiring", () => {
    it("editor.action.triggerSuggest исполняется без ошибок", () => {
        const h = createAppTestHarness();
        h.controller.openFile("/tmp/completion-trigger.txt");
        h.controller.focusEditor();
