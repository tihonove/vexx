...
// the leftmost gutter column marks changed lines. This core chunk has no git
// wiring yet, so the scenario pushes decorations straight to the active editor
// with literal packed-RGB colours (VS Code's dirty-diff defaults) and captures
// the painted gutter: added lines green, modified blue, a deleted-hunk boundary
...
export default defineScenario({
    ...
    async run(editor) {
        ...
        await editor.setGutterChangeDecorations([
            { startLine: 1, endLine: 1, color: MODIFIED },
