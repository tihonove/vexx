...
- [x] **WP7** — полная поверхность `workspace.fs`
      (`stat`/`readDirectory`/`createDirectory`/`readFile`/`writeFile`/`delete`/`rename`/`copy`/`isWritableFileSystem`)
      + `openTextDocument(uri, {encoding})` + сквозная команда `EditorConfig.generate`.
      `workspace.fs` реализован **локально через `node:fs`** в subprocess (целевой файл —
      на той же машине, не открытый буфер), без RPC. `FileSystemError` маппит
      `ENOENT`/`EEXIST`/`ENOTDIR`/`EISDIR`/`EACCES`/`EPERM` + фабрики
      `FileNotADirectory`/`FileIsADirectory`. Disk-read `openTextDocument` детектит EOL из
      содержимого (`encoding` остаётся `utf8` — реальное транскодирование charset-bound, вне
      объёма). Оставшийся техдолг #49: реальная перекодировка не-utf8 при чтении (зависит от
      charset-поддержки в ядре).
...
читает файл как utf-8 и строит эфемерный документ (не в реестре); `eol` теперь
детектится из содержимого (LF/CRLF мажоритарно), но `encoding` остаётся `"utf8"` —
параметр `encoding` принимается для совместимости с API 1.100, при несовпадении —
graceful degrade с предупреждением `window.showMessage`. Полноценное транскодирование
не-utf8 кодировок в ядре — вне объёма (см. EOL-модель WP5 и будущую работу по charset).
