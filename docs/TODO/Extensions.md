...
- [x] Completion editor-focus (#52): переход с popup-focus на **editor-focus** модель (как в VS Code). Попап больше не забирает фокус/клавиатуру — редактор сохраняет фокус, печать идёт в буфер, а `CompletionController` живо рефильтрует список по префиксу под кареткой (подписка на `onDidChangeContent`/`onDidChangeCursorPosition`) и переезжает вслед за кареткой. Навигация (`↑↓/Enter/Tab/Esc`) перехватывается `AppController` через suggest-экшены с `when: suggestWidgetVisible`; остальные клавиши падают в редактор. Тесты: `AppController.Suggest.test.ts`, `CompletionController.test.ts`, `CompletionListElement.test.ts`.

Остаётся:

- [ ] Completion trigger-characters (#52, подфича): авто-открытие попапа по `triggerCharacters` из `registerCompletionItemProvider(..., ...triggerCharacters)` (напр. `.`/`=`). Нужна проброска набора триггер-символов из subprocess в ядро (через `languages.updateSubscriptions`-мету → `ExtensionHost` → `EditorGroupController`) и авто-`trigger()` в контроллере при вводе триггер-символа. Сейчас попап открывается только явно (Ctrl+Space).
