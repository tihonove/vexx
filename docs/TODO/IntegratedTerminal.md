...
- **Workbench (glue с нативом)** — `src/Workbench/Services/Terminal/`:
  ...
  - `TerminalPanelComponent.ts` (в `src/Workbench/Components/Panel/`) — виджеты `TerminalViewElement` по инстансам, вкидывание активного в вкладку.
...
Реализация:
- `src/Workbench/Services/Terminal/loadNodePty.ts` — dev: `require("node-pty")`; SEA: `sea.getAsset` → распаковка → `createRequire`.
...
- **e2e-скриншот**: `e2e/scenarios/terminal.scenario.ts` (в `npm run test:e2e` и `npm run screenshots`) —
  ...
  Юнит/интеграция — `TerminalService.test.ts`, `TerminalPanelComponent.test.ts`, `AppController.Terminal.test.ts`,
