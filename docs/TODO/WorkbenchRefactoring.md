...
1. **Контролы** (примитивы TUIDom) — переиспользуемая «вещь в себе»: не знают ничего про темы,
   ...
   (packed-цвета) через `options.styles` конструктора и/или метод `style()`, плюс
...
- [ ] 1. Стандарт `IXxxStyles` + дестемизация контролов TUIDom (Button-эталон; MenuBar/PopupMenu/
...
class ButtonElement {
    ...
    style(styles: IButtonStyles): void; // единственный канал обновления, вызывает markDirty()
}
```
...
подписан на смену темы (`ThemedComponent.updateStyles()`) и заново вызывает `control.style(...)`.
