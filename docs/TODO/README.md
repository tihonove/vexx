...
- [~] [Diff](Diff.md) — дифф-редактор и вкладка Changes; алгоритм построчного диффа вендорнут из upstream (`DefaultLinesDiffComputer` + корпус на 58 фикстур) — далее абстракция editor input/pane, буфер не с диска, форма вью со свёрнутыми регионами
