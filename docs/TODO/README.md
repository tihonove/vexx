...
- [~] [ReadonlyEditor](ReadonlyEditor.md) — режим «только чтение» у редактора (аналог `EditorOption.readOnly`); флаг + гейт команд + замок на вкладке готовы; далее — detached pane (уезжает в Output) и конфиг-слой `files.readonly*`
- [~] [Logging](Logging.md) — единый ILogService + RingBufferSink/FileSink (Phases 1–3.5 готовы); далее Output UI (ждёт detached pane из [ReadonlyEditor](ReadonlyEditor.md)), CLI flags, vscode API
