...
- **`files.encoding`** — кодировка по умолчанию для открытия/сохранения (вместо
  захардкоженного utf-8), применять в `EditorService.applyConfigurationToEditor` (`src/Workbench/Services/EditorService.ts`).
...
Файлы: `src/Editor/Encoding.ts`, `src/Workbench/Services/TextFile/TextFileModel.ts`,
`src/Workbench/Services/EditorService.ts`.
