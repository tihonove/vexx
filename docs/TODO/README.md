...
- [~] [Diff](Diff.md) — дифф-редактор и вкладка Changes. План разбит на 7 этапов, каждый отгружается отдельно. Этапы 0 (вендоринг `DefaultLinesDiffComputer` + корпус на 58 фикстур) и 1 (живой гуттер: реестр провайдеров ФС по схеме, `git:` в builtin-расширении, `QuickDiffService` — бары двигаются при наборе, до сохранения) готовы; следующий — этап 2, diff view model со свёрнутыми регионами
