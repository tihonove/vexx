...
- [~] [LSP](LSP.md) — **встроено (dev)**: `typescript-language-server` через `vscode-languageclient` даёт диагностики в обычном `npm start` (squiggle + Problems, выкл `VEXX_LSP_DISABLE=1`); застабали `vscode`-поверхность + сток диагностик в MarkerService + core→host document sync; далее — SEA/prod-deps, ленивая активация, тесты (вернуть coverage), hover/definition в UI
