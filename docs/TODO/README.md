...
### [ ] В инпутах приложения нет выделения и буфера обмена
Экшены `inputSelectLeft/Right/ToHome/ToEnd/WordLeft/WordRight/All` и
`inputCopy/inputCut/inputPaste` (`src/Controllers/Actions/InputActions.ts`) и
соответствующие методы `InputWidgetController` реализованы и работают, но
зарегистрированы **только** в `InputElement.stories.ts` — в самом приложении
`AppController` (`builtinActions`) подключает лишь cursor/delete. Из-за этого в
реальных инпутах (QuickOpen, поиск и т.п.) нет Shift-выделения и Ctrl+C/X/V.
Надо зарегистрировать эти экшены там, где приложение фокусирует `InputElement`
(под `when: "inputWidgetFocus"`), и проверить интеграционным тестом через `TestApp`.
