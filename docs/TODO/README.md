...
- [~] [Diff](Diff.md) — дифф-редактор и вкладка Changes. План разбит на 7 этапов, каждый отгружается отдельно. Этап 0 (вендоринг `DefaultLinesDiffComputer` из upstream + корпус на 58 фикстур) готов; следующий — этап 1, живой гуттер: вычисление диффа переезжает в основной процесс, change-bars перестают залипать до сохранения
