...
- **Editor-write seam (#194):** `TextEditor.edit(cb)` (с `TextEditorEdit` insert/replace/delete), `editor.selection(s)` геттер/сеттер, `window.visibleTextEditors`, value-тип `Selection`. Правки/выделения субпроцесса едут хосту (`editor.applyEdit` / `editor.setSelection`) и применяются к активному редактору порта `IEditorOptionsService` (правки — одним undoable-батчем через `applyExternalEdits`, с клампом к границам; guard по совпадению uri). Первичное выделение проецируется в subprocess в `IActiveEditorMeta`.
