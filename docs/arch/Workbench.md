...
`OpenFileCommandContribution` (команда `workbench.openFile`). Все — `restored`.

## MenuRegistry (`src/Workbench/Menus/`)
...
лишь запрашивает готовый список.

**Контракт.** `MenuId` (`EditorContext`/`ExplorerContext` — точки, куда
контрибьютятся пункты). `IMenuContribution { menuId; command; title?; when?;
visible?; group?; order?; icon?; args?; shortcut? }` — декларативная запись
(зеркало `builtinActions`). Явный массив `MENU_CONTRIBUTIONS`.

**Реестр.** `MenuRegistry.getMenuItems(menuId, context?)`: фильтр по `when`
(`ContextKeyService.evaluate`) и по `visible(context)`, сортировка по group
(строка) + order, авто-разделители **между непустыми группами** (скрытый пункт
схлопывает группу без лишнего сепаратора), маппинг в `MenuEntry`:
...
`MenuRegistry` — чистая функция реестров команд/кейбиндов/контекст-ключей;
состояние открытия (напр. буфер обмена файлов, путь узла) приходит параметром
`context`, не через DI. Конвенция контекста: `EditorContext → undefined`;
`ExplorerContext → { path, canPaste }`.

**Границы vs contribution/Action.** Menu-contribution — *где показывать команду*
(декларативные данные). Command+keybinding — `Actions/` (поведение). Event-проводка
— `Contributions/`.

**Ленивый резолв редактора.** Контекст-меню редактора берётся не статическим
массивом, а провайдером `EditorElement.contextMenuProvider` (резолвится в момент
ОТКРЫТИЯ — честная `when`-фильтрация); ставит его `EditorContextMenuContribution`.
Explorer резолвит меню при каждом открытии (Paste виден по `canPaste`).

Охват: контекст-меню редактора и Explorer. Меню-бар (`MenuService`) — на реестр
пока не переведён (follow-up).
