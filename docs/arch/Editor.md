...
**Стили.** Специализированные цвета редактора (гуттер, номера строк, word-highlight, indent-guides, fold-контрол, squiggles, стили контекстного меню `menu`) приходят единым каналом `EditorElement.setStyles(IEditorStyles)` по общему стандарту контролов (`unthemedEditorStyles` — baseline без темы). Маппинг тема → стили — `getEditorStyles` в `src/Workbench/Styles/defaultStyles.ts`; вызывает его `EditorComponent.updateStyles` (Workbench; повторно применяет кэш при пересоздании `EditorElement` после перечитки модели с диска). Основные fg/bg редактора идут отдельным легальным каналом — `editor.style = { fg, bg }` (система наследования TUIStyle).
