...
- **Статус-бар** — `StatusBarController` ↔ `StatusBarElement`. Правые сегменты в порядке VS Code: `Ln X, Col Y` · Encoding (`statusLabel` из `SUPPORTED_ENCODINGS`) · EOL (`LF`/`CRLF`) · Language. Айтемы кликабельны через `StatusBarItem.onClick` — колбэк, а не command-id (TUIDom не знает про команды): замыкания строит `StatusBarController` и исполняет команды через `CommandRegistry` (`workbench.action.editor.changeEncoding` / `changeEOL`). Пикеры этих команд (двухуровневый Reopen/Save with Encoding и LF/CRLF) живут в `AppController` поверх `QuickInputController`; сегменты подписаны на `onDidChangeEncoding`/`onDidChangeEol` активного редактора.
