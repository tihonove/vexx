...
## Подсистемы
- **`IFileWatcher`** (`IFileWatcherDIToken`) — слежение за отдельным файлом; реальная `ChokidarFileWatcher`, заглушка `NULL_FILE_WATCHER`. `EditorGroupController` инжектит его в каждый `EditorController` (детект внешних изменений, см. [Editor.md](Editor.md)). Дерево файлов слушается отдельно в `FileTreeDataProvider`.
