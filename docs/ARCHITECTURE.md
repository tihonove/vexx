...
- **themes/** — встроенные темы, импортированные **verbatim** из microsoft/vscode скриптом `scripts/import-vscode-themes.mjs` (резолвит `include`-цепочку, флэттит в один `IThemeFile`; пин тега — тот же, что у языковых паков). Набор: Dark Modern (дефолт), Dark+, Monokai, Light Modern, Light+. `builtinThemes.ts` — порядок для пикера + `DEFAULT_COLOR_THEME`. Темы **разреженные** (как в VS Code): workbench-цвета, которые они не переопределяют, приходят из `defaultColors.ts` — поэтому дописывать цвета руками в эти файлы не нужно (перезатрётся при ре-импорте).
