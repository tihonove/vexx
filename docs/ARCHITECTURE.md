...
3. **Workbench** — прикладной слой приложения: Services (логика) + Components (UI-сборка, корень — `WorkbenchComponent`) + Contributions (фич-проводка вне корня, по фазам) + Actions (встроенные команды) + Styles (мост тема → стили контролов) + Modules (DI-проводка)
