...
### Controllers/
Контроллеры с чётким lifecycle (constructor → mount → activate → dispose) и `Modules/` (модули и профили DI). Слой **поэтапно растворяется в Workbench** (логика → `Workbench/Services/`, UI-сборка → `Workbench/Components/`; план — [TODO/WorkbenchRefactoring.md](TODO/WorkbenchRefactoring.md)): система команд, файловый watcher, `Workspace/`, `TerminalEnvironment/`, `Terminal/`, `Diagnostics/` уже переехали в `Workbench/Services/`; статус-бар мигрировал целиком — эталонная пара `StatusBarService` + `StatusBarComponent` (`Workbench/Components/StatusBar/`); Panel-кластер (диагностики, нижняя панель, Problems, встроенный терминал) — `DiagnosticsService`, `PanelService`+`PanelComponent`, `ProblemsComponent`, `TerminalService`+`TerminalPanelComponent` (`Workbench/Components/Panel/`); QuickInput-кластер — `QuickInputService`+`QuickOpenService` поверх общего `QuickInputComponent` (`Workbench/Components/QuickInput/`); Editor-кластер — `TextFileModel` (`Workbench/Services/TextFile/`) + `EditorComponent`/`EditorPane`/`EditorGroupComponent` (`Workbench/Components/Editor/`) поверх `EditorService`; с этапа 11 — Shell-кластер (`MenuService`+`MenuBarComponent` в `Workbench/Components/Shell/`, `LayoutService`, `WorkbenchStateService`, `WorkbenchContextKeys`), а каталог экшенов живёт целиком в `Workbench/Actions/` (включая `builtinActions.ts`). В Controllers остались `AppController` (тонкая скорлупа: корневая view + bootstrap) и `Modules/`. Детали → [arch/Controllers.md](arch/Controllers.md).
