...
- **WorkbenchTheme** — основной класс с полями `name`/`type` и методами `getColor(key)` / `getColorOrDefault(key, default)`, статический `fromThemeFile(json)` парсит hex→packRgb при загрузке
...
- **themes/** — встроенные темы, импортированные **verbatim** из microsoft/vscode скриптом `scripts/import-vscode-themes.mjs` (резолвит `include`-цепочку, флэттит в один `IThemeFile`; пин тега — тот же, что у языковых паков). Набор: Dark Modern (дефолт), Dark+, Monokai, Light Modern, Light+. `builtinThemes.ts` — порядок для пикера + `DEFAULT_COLOR_THEME`.
