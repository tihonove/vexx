...
- **`Extensions/Host/Vscode/`** — субпроцессная поверхность `vscode`:
  ...
  - **`ExtHostDocuments.ts`** — `DocumentRegistry` (`Map<fileName, ExtHostTextDocument>`, `getOrCreate`/`upsertMeta`/`upsertFull`) со **стабильной идентичностью**: один объект на `fileName` живёт всю сессию, обновления мутируют его на месте (нужно для `activeTextEditor.document === doc` по ссылке). `ExtHostTextDocument` — `fileName`/`uri`/`languageId`/`isDirty`/`version`/`eol`/`getText`/`lineCount`/`lineAt→TextLine`; текст лениво из последнего снапшота (полный текст + `eol` — на путях will-save, completion и disk-read `openTextDocument`, где `eol` детектится из содержимого; `encoding` всегда `utf8` — ядро utf-8-only). `lineAt`/`getText` разбивают строки EOL-агностично (`\r\n`|`\n`), хвостовой `\r` в текст строки не протекает.
  ...
  - **`FileSystemNamespace.ts`** — полная поверхность `workspace.fs` (`stat`/`readDirectory`/`createDirectory`/`readFile`/`writeFile`/`delete`/`rename`/`copy`/`isWritableFileSystem`) **локально через `node:fs`** (без RPC — целевой файл на той же машине); ошибки `node` маппятся в `FileSystemError` с тем же `code` (`ENOENT`→FileNotFound, `EEXIST`→FileExists, `ENOTDIR`→FileNotADirectory, `EISDIR`→FileIsADirectory, `EACCES`/`EPERM`→NoPermissions). Байтовая поверхность полна; транскодирование charset/EOL — вне объёма (ядро utf-8/LF-only).
