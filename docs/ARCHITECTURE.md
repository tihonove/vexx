...
3. **Workbench** — компоненты приложения (модель Service ↔ Component, как в vscode): владеют view из контролов TUIDom, получают сервисы в конструктор; сюда постепенно мигрируют view-контроллеры
4. **Controllers** — контроллеры с lifecycle (constructor → mount → activate → dispose), оркестрация UI и бизнес-логики; по мере распила роль слоя сужается до **сервисов** (app-логика без view)
5. **Configuration** — настройки пользователя (JSONC, профили, слои default/user/profile)
6. **Theme** — темизация (VS Code-совместимые theme files); на одном уровне с Controllers, подключается к Editor через интерфейсы
7. **Editor** — модель текстового редактора + мост к TUIDom
8. **TUIDom** — TUI-фреймворк (аналог браузерного DOM): дерево элементов, события, контролы-примитивы; **тема-независим** («вещь в себе»)
...
### TUIDom/
TUI-фреймворк — дерево элементов с layout, событиями, фокусом (аналог браузерного DOM). `RenderContext`, система событий (capture/bubble, default actions), стилей (наследование fg/bg), контролы-примитивы и `OverlayLayer` (session API с обязательным `pointerPolicy`). **Инвариант: TUIDom не знает про темы и прочие app-концепции** — контролы несут только plain color-props с theme-less дефолтами; маппинг темы в цвета живёт выше (`applyXxxTheme`-хелперы, компоненты Workbench). Среди виджетов — `Widgets/Terminal/` (`TerminalViewElement` рендерит абстрактную `ITerminalSurface`; чистый — без импортов PTY/эмулятора). Layout описан в [LAYOUT.md](LAYOUT.md). Детали → [arch/TUIDom.md](arch/TUIDom.md).

### Workbench/
Слой компонентов приложения (аналог vscode workbench). Модель **Service ↔ Component**: сервис — app-логика без view (`DialogService`), компонент — владеет корневым контролом TUIDom, получает сервисы в конструктор и подписывается на них; в жизненный цикл контролов не встраивается (не наследует `TUIElement`), а размещает их как DOM-узлы. База — `Component` (тема через хук `applyStyles`), контракт — `IComponent`. Живут здесь `StatusBarComponent` и `Dialogs/` (`DialogService`, `AboutDialog`, `ConfirmDialog`, `ConfirmSaveDialog`). Детали → [arch/Workbench.md](arch/Workbench.md).
...
```
App → Extensions → Workbench → Controllers → Editor → TUIDom → { Input, Rendering, Backend } → Common
          ↑            ↑           ↑             ↑
...
- **TUIDom** зависит от Rendering, Common (через TerminalScreen). **Не зависит от Theme** — контролы несут plain color-props с theme-less дефолтами; маппинг темы в цвета живёт выше (`applyXxxTheme`-хелперы в Controllers, `applyStyles` компонентов Workbench)
...
- **Workbench** зависит от Controllers (сервисы), Editor, Theme, TUIDom, Common. Переходные исключения в обратную сторону (до распила `AppController`): `AppController` импортирует Workbench-компоненты, `Controllers/Modules/` (composition root) биндит их в DI. Детали → [arch/Workbench.md](arch/Workbench.md)
...
Примитивы DI (`Token`, `Container`, `token()`) реализованы в `Common/DiContainer.ts`, но **объявлять конкретные DI-токены и импортировать `Container`** можно **только на уровнях Controllers, Workbench и App**. Слои ниже (Editor, TUIDom, Input, Rendering, Backend) не должны зависеть от DI-контейнера.
