...
### Controllers/
Контроллеры с чётким lifecycle (constructor → mount → activate → dispose), диагностики (`DiagnosticsController` + нижняя Panel/Problems), встроенный терминал (`TerminalController`), каталог `Actions/` (команды приложения) и `Modules/` (модули и профили DI). Слой **поэтапно растворяется в Workbench** (логика → `Workbench/Services/`, UI-сборка → `Workbench/Components/`; план — [TODO/WorkbenchRefactoring.md](TODO/WorkbenchRefactoring.md)): система команд, файловый watcher, `Workspace/`, `TerminalEnvironment/`, `Terminal/`, `Diagnostics/` уже переехали в `Workbench/Services/`; статус-бар мигрировал целиком — эталонная пара `StatusBarService` + `StatusBarComponent` (`Workbench/Components/StatusBar/`). Детали → [arch/Controllers.md](arch/Controllers.md).
