...
Источник областей — `FoldingRangeProvider.computeIndentationFolds(document, tabSize)`: дефолтный провайдер по отступам (как в VS Code, offSide пока не учитывается). `EditorController` пересчитывает области при открытии и (дебаунсом через микротаск, после того как view-state сдвинул существующие области под правку) на каждый `onDidChangeContent`, сохраняя `isCollapsed` для областей с тем же `startLine`. Расширенческий провайдер (`languages.registerFoldingRangeProvider`, #87) — будущий seam поверх этой точки. Рендер: `EditorElement` рисует chevron в правой колонке гуттера (down — раскрыто, right — свёрнуто) и inline-маркер `⋯` после заголовка свёрнутой области; клик по колонке chevron'а тоглит область. Цвет — `editorGutter.foldingControlForeground`. Команды и бинды (`editor.fold`/`unfold`/`toggleFold`/`foldAll`/`unfoldAll`) — в `Controllers/Actions/FoldingActions.ts`.
