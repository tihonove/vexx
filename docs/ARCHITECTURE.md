...
`GridSnapshot.ts` — plain-data сериализация кадра (`snapshotGrid(grid, cursor)` → `{ cols, rows, cursor, cells[] }`, ячейка = `{ char, fg, bg, style, width }`). Нужна, чтобы перерисовать экран вне терминала (растеризатор картинки, diff-вьюер) и передать кадр по инспектор-протоколу без графических зависимостей в редакторе.

`gridToSvg.ts` — чистая (pure-TS, без шрифтов/библиотек) сборка `GridSnapshot` → самодостаточный SVG: фон — merged-`<rect>` по прогонам bg, глифы — `<text>` по прогонам одного стиля с `textLength`/`lengthAdjust`, так что прогон точно заполняет свой столбцовый диапазон и сетка не «плывёт» под любым шрифтом. `Inverse` меняет fg/bg местами, широкие ячейки занимают 2 столбца. Растеризация SVG→PNG живёт **только в тулинге** (`e2e/helpers/renderScreenshot.ts`, resvg + системный шрифт), не в редакторе.
...
`RenderContext.drawBox(x, y, width, height, options?)` — общий хелпер отрисовки рамки: рисует углы, горизонтали и вертикали одним вызовом, поддерживает `fill` (заливка фона), `separators` (ряды-разделители `├───┤`) и пресеты стилей из `BorderStyle.ts` (`BORDER_ROUNDED` — канон nvchad-стиля, `BORDER_SINGLE`, `BORDER_DOUBLE`). Все бордер-виджеты (`InputElement`, `FindWidgetElement`, `BoxContainerElement`, `PopupMenuElement`, `QuickPickElement`, `CompletionListElement`) рисуют рамку через него — единый стиль углов, без дублированных циклов.
...
Модель фолдинга живёт в `EditorViewState` как «линза» проекции документа на видимые строки. `IFoldingRegion { startLine, endLine, isCollapsed }` — свёрнутая область прячет строки `startLine+1 .. endLine`, заголовок остаётся видимым. API view-state: `setFoldingRegions`, `toggleFold(line)` (по заголовку), `foldRegionContaining/unfoldRegionContaining/toggleFoldContaining(line)` (по внутренней области у курсора), `foldAll/unfoldAll`, `ensureLineVisible`. Курсорная навигация и `revealRange` пропускают/раскрывают свёрнутые области; `adjustFoldingRegionsForEdits` сдвигает границы при правках.

Источник областей — `FoldingRangeProvider.computeIndentationFolds(document, tabSize)`: дефолтный провайдер по отступам (как в VS Code, offSide пока не учитывается). `EditorController` пересчитывает области при открытии и (дебаунсом через микротаск, после того как view-state сдвинул существующие области под правку) на каждый `onDidChangeContent`, сохраняя `isCollapsed` для областей с тем же `startLine`. Расширенческий провайдер (`languages.registerFoldingRangeProvider`, #87) — будущий seam поверх этой точки. Рендер: `EditorElement` отводит под фолдинг-контрол 3-колоночную зону в конце гуттера (зазор — codicon-шеврон в `foldControlColumn` — зазор перед текстом; `down` — раскрыто, `right` — свёрнуто) и inline-маркер `⋯` после заголовка свёрнутой области; клик по `foldControlColumn` тоглит область. Шеврон **раскрытого** региона (`down`) рисуется только пока мышь наведена на гуттер (флаг `foldGutterHovered`, взводится на `mousemove` при `localX < gutterWidth`, гасится на `mouseleave`) — как VS Code `editor.showFoldingControls: "mouseover"`; шеврон **свёрнутого** региона (`right`) виден всегда. Цвет — `editorGutter.foldingControlForeground`. Команды и бинды (`editor.fold`/`unfold`/`toggleFold`/`foldAll`/`unfoldAll`) — в `Controllers/Actions/FoldingActions.ts`.

Indent-guides: `EditorElement.paintIndentGuides` рисует вертикальную линию `│` над leading-whitespace каждого региона (на колонке `computeIndentLevel(header)`, по строкам тела `startLine+1..endLine`) — как в VS Code/nvchad. Пасс идёт после контента, но до bg-подсветок (selection/find кладут только `bg`, сохраняя глиф → линия видна и под выделением). Активная линия (ближайший охватывающий курсор регион — последний в отсортированном по `startLine` `foldedRegions`) рисуется ярче. Цвета — `editorIndentGuide.background1` / `editorIndentGuide.activeBackground1` (дефолты `#404040`/`#707070`).
...
Провайдер-агностичный реестр диагностик по образцу VS Code `IMarkerService` (`vs/platform/markers`). Диагностики **отвязаны от источников**: язык-серверы, problem matchers и расширения (`languages.createDiagnosticCollection`) — лишь *поставщики*, пишущие в реестр; панель Problems, squiggle в редакторе и счётчики — *потребители*, читающие из него. Поэтому рабочий MVP собирается без LSP/matchers — достаточно одного поставщика.

- **`IMarker`/`IMarkerData`/`MarkerSeverity`** (`Hint|Info|Warning|Error`, числовые значения как в VS Code) — модель. `IMarkerData` (severity, range, message, code?, source?) даёт поставщик; `IMarker` — это `IMarkerData` + `owner` (неймспейс поставщика) + `resource` (путь файла).
- **`MarkerService`** — реестр `owner → resource → IMarker[]`: `changeOne(owner, resource, markers)` (пустой массив — очистка), `read(filter?)` (по owner/resource/severities), `onDidChangeMarkers(resources)`. Чистый класс без DI — зеркало `TokenizationRegistry`; биндится в Controllers (`MarkerServiceDIToken`, `markersModule`).
- **`IMarkerDecoration`** — view-проекция маркера для рендера squiggle (только `range` + `severity`), чтобы Editor не зависел от `owner/resource/message`.

Рендер squiggle: `EditorElement.markerDecorations` красит покрытые диапазоном ячейки severity-цветом + `StyleFlags.Undercurl` (SGR `4:3`; на legacy-терминалах остаётся цвет). Общий обход диапазона (`forEachRangeCell`) переиспользуется с фоновыми подсветками (selection/find). Цвета — `editorError/Warning/Info/Hint.foreground` (дефолты в `defaultColors.ts`), прокидываются в `EditorController.applyTheme`, fallback — встроенные константы VS Code dark. Точка пересчёта — `ITextDocument.onDidChangeContent` (см. выше «marker tracking»). Проводка «производство → реестр → потребление» — в `DiagnosticsController` (слой Controllers). Дорожная карта — [docs/TODO/Problems.md](TODO/Problems.md).
...
- **`parseCliArgs(argv)`** — CLI-парсер (`Common/CliArgs.ts`): флаги `--user-data-dir <path>`, `--profile <name>`, `--inspect-tui[=host:port]`, `--headless[=<cols>x<rows>]` (запуск без терминала через `HeadlessCaptureBackend`; требует `--inspect-tui`), `-h/--help`, `-v/--version`, разделитель `--`, неизвестные флаги → `CliArgsError`.
...
- **`IConfigurationService`** + **`ConfigurationService extends Disposable`** — `loadConfiguration(paths)` async-bootstrap, читает оба слоя через `jsonc-parser`, отсутствующий файл → EMPTY. `onDidChangeConfiguration` — пока no-op (watcher не реализован, изменения подхватываются перезапуском). **`updateUserValue(key, value)`** (опциональный метод интерфейса) — запись настройки в settings.json активного профиля через `jsonc-parser.modify`/`applyEdits` (сохраняет комментарии и форматирование; плоский dotted-ключ, как в VS Code) + синхронное обновление in-memory слоя. Цель записи — `paths.settingsFile` (default → `User/settings.json`, именованный профиль → его файл). Первый потребитель — theme picker (`workbench.colorTheme`). Заглушки (`NULL_CONFIGURATION_SERVICE`) метод не реализуют — вызов через optional chaining.
...
### Theme/
Система темизации, совместимая с VS Code theme files. Активная тема — объект `WorkbenchTheme` за `ThemeService` (`ThemeServiceDIToken`), хранящий packed RGB цвета и правила подсветки синтаксиса. Контроллеры применяют цвета к элементам через `applyTheme()` и переприменяют их по `ThemeService.onThemeChange`; TUIDom ничего не знает о темах.

> **Правило работы с цветами (единственная система цветов).** Все цвета UI берутся **только** из активной темы через `theme.getColor(key)` / `theme.getRequiredColor(key)`. Никаких цветовых литералов в контроллерах/виджетах и никаких инлайн-фоллбэков (`getColorOrDefault(key, packRgb(...))` больше нет). Цвета, которых нет в JSON-темах VS Code (они приходят из code-based color registry), задаёт наш реестр дефолтов `defaultColors.ts`. По мере добавления фич цвета растут **в одном месте**: раскрыть ключ в `IWorkbenchColors` + добавить дефолт (dark/light) в `defaultColors.ts`. Исключение — widget-baseline константы для элементов без темизируемого ключа VS Code (например `menu` shortcut fg, рамка/счётчик find-виджета).

- **IThemeFile** — типизация для theme JSON (формат совместим с VS Code 1:1)
- **IWorkbenchColors** — интерфейс со всеми ~700 цветовыми ключами VS Code (большинство закомментировано, раскрываются по мере реализации)
- **IEditorTokenTheme** — правила подсветки синтаксиса (TextMate token colors)
- **WorkbenchTheme** — основной класс с полями `name`/`type`. `getColor(key)` → `number | undefined` (для опциональных цветов, которые потребитель обрабатывает сам); `getRequiredColor(key)` → `number`, **кидает**, если цвета нет ни в теме, ни в реестре дефолтов (enforcement правила: используемый цвет обязан иметь дефолт). Статический `fromThemeFile(json)` слоит реестр дефолтов **под** цвета темы (`{ ...defaults[kind], ...json.colors }`, тема побеждает) и парсит hex→packRgb.
- **defaultColors** — реестр workbench-дефолтов (аналог VS Code default color registry) по kind (`dark`/`light`). Заполняет цвета, которых нет в verbatim-темах VS Code, чтобы `getColor`/`getRequiredColor` резолвились на **любой** теме. Покрытие всех required-цветов на всех встроенных темах проверяет `defaultColors.test.ts`.
- **ColorUtils** — `parseHexColor()` — конвертация hex-строк (#RGB, #RGBA, #RRGGBB, #RRGGBBAA) → packed RGB
- **ThemeService** — держит активную `WorkbenchTheme`, `setTheme(theme)` + `onThemeChange(listener)` (immediate + on-change). Смена темы here → live re-apply во всех контроллерах.
- **ThemeRegistry** (`ThemeRegistryDIToken`) — реестр доступных тем по `label` (= `IThemeFile.name`). `resolve(label) → WorkbenchTheme`, `list() → IThemeDescriptor[]` (label + base type для пикера). `createBuiltinThemeRegistry()` сидит все встроенные темы. Точка расширения под `contributes.themes` от расширений (см. [docs/TODO/Theming.md](TODO/Theming.md)).
- **themes/** — встроенные темы, импортированные **verbatim** из microsoft/vscode скриптом `scripts/import-vscode-themes.mjs` (резолвит `include`-цепочку, флэттит в один `IThemeFile`; пин тега — тот же, что у языковых паков). Набор: Dark 2026, Dark Modern (дефолт), Dark+, Monokai, Light Modern, Light+. `builtinThemes.ts` — порядок для пикера + `DEFAULT_COLOR_THEME`. Темы **разреженные** (как в VS Code): workbench-цвета, которые они не переопределяют, приходят из `defaultColors.ts` — поэтому дописывать цвета руками в эти файлы не нужно (перезатрётся при ре-импорте).

Выбор активной темы: `main.ts` читает `workbench.colorTheme` из конфига → `ThemeRegistry.resolve(label)` (неизвестное имя → откат на `DEFAULT_COLOR_THEME`). Смена в рантайме — команда `workbench.action.selectTheme` (`AppController.selectColorTheme`): quick-pick со **live preview** (тема применяется по мере навигации по списку), Enter — применяет и persist'ит в `workbench.colorTheme`, Escape — откат на исходную. Persist делает `IConfigurationService.updateUserValue` (см. Configuration/).
...
**`DiagnosticsController`** (`Controllers/DiagnosticsController.ts`) — headless-обозреватель (без `view`), проводка диагностик между поставщиками и потребителями поверх `MarkerService` (см. Editor/Markers). MVP: единственный поставщик — встроенный валидатор `settings.json` (`Controllers/Diagnostics/SettingsDiagnostics.ts`: `parseTree` из `jsonc-parser` → неизвестные top-level ключи → Warning-маркеры «Unknown Configuration Setting» с диапазоном из node offsets; набор известных ключей — `collectKnownSettingKeys(getDefaultConfiguration())`, т.е. flatten дефолтов Configuration до dotted-путей + префиксов). Валидируется **только активный settings.json Vexx** — матч по точному пути (`SettingsResourceDIToken` = `IUserDataPaths.settingsFile`, прокинутый из `main.ts`), а не по basename, чтобы чужой `settings.json` (например от VS Code или воркспейсный `.vscode/settings.json`) не подсвечивался. Единственный потребитель — squiggle в редакторе: контроллер слушает `EditorGroupController.onActiveEditorChanged` (+ `onDidChangeContent` активного редактора) → валидирует → `MarkerService.changeOne`; и `MarkerService.onDidChangeMarkers` → пушит декорации в открытые редакторы совпавшего ресурса через `EditorController.setMarkerDecorations`. LSP/problem matchers/расширения — будущие поставщики того же реестра, панель Problems — будущий потребитель ([docs/TODO/Problems.md](TODO/Problems.md)).

**`PanelController`** (`Controllers/PanelController.ts`) — владеет нижней **Panel** (VS Code `ViewContainerLocation.Panel`) через `PanelContainerElement` (TUIDom-виджет: плоская верхняя полоска-разделитель `panel.border`; таб-шапка Views с отступом и приглушённым текстом `panelTitle.inactiveForeground` — активная вкладка подчёркиванием ровно под глифами; активный View / empty-state ниже, с левым отступом под текст таба).

**`ProblemsController`** (`Controllers/ProblemsController.ts`) — наполняет вкладку Problems деревом «файл → маркеры» (второй потребитель `MarkerService`; первый — squiggly в редакторе). Дерево — `TreeViewElement<ProblemNode>` поверх `ProblemsTreeDataProvider` (`Controllers/Diagnostics/ProblemsTreeDataProvider.ts`: группирует маркеры по ресурсу, сортирует по severity/строке; file-узел «basename (N)», marker-лист «сообщение [Ln, Col]» с codicon-иконкой severity и цветом `editorError/Warning/Info/Hint.foreground`). Подписан на `MarkerService.onDidChangeMarkers`: 0 маркеров → `setViewContent(PROBLEMS_VIEW_ID, null)` (панель показывает placeholder-empty-state), иначе — дерево + `refresh` с авто-разворотом файлов. Enter/клик по маркеру → `EditorGroupController.openFile` + `EditorController.goToPosition/revealRange` (reveal). Ctrl+Shift+M (`workbench.actions.view.problems`) открывает панель и **фокусирует** дерево. Цвета дерева — `list.*` + `panel.background`. Провайдер (`ProblemsTreeDataProvider`) держит file-узлы **без встроенных данных** — маркеры ищутся по `resource` из текущего снапшота, поэтому устаревшая ссылка на узел (её может вернуть `TreeViewElement` при refresh) резолвит свежие маркеры.

**Нижняя Panel и провязка root/стиля.** Скрытая панель исключена из `WorkbenchLayoutElement.getChildren()`, а `root`/стиль распространяются только по `getChildren()`. Панель скрыта по умолчанию, поэтому на старте не получает `root` и не резолвит стиль. Чтобы дерево красилось, фокусировалось и реагировало на клавиши, `setBottomPanelVisible(true)` на переходе в видимое **пере-подключает** панель: `bottomPanel.setParent(this)` (пробрасывает текущий `root` вниз) + `bottomPanel.markStyleDirty()` (публичный; форсит переразрешение стиля поддерева). MVP: одна вкладка Problems (`workbench.panel.markers.view`) с empty-state placeholder — дерево маркеров подключится сюда как `content` (следующий этап). Видимостью Panel владеет `WorkbenchLayoutElement` (нижняя зона, выровненная по ширине редактора, с горизонтальным `SashElement`; скрыта по умолчанию); `AppController` тоглит её командами `workbench.action.togglePanel` (Ctrl+J) и `workbench.actions.view.problems` (Ctrl+Shift+M), синхронизируя контекст-ключ `panelVisible`. Цвета — `panel.*`/`panelTitle.*`.
