...
## WorkbenchLayoutElement — левая панель и ресайз

`WorkbenchLayoutElement` — двухколоночный split: слева панель (explorer), справа центр
(редактор). Ширина панели — **абсолютная**, в колонках (`leftPanelWidth`, по умолчанию 30),
поэтому при ресайзе терминала панель сохраняет свой размер, а перетекает только редактор.

Изменять ширину можно:
- мышью — перетаскиванием границы (см. `SashElement` ниже);
- командами палитры `workbench.action.increase/decrease/resetSidebarWidth` (шаг 3, дефолт 30).

Клэмп ширины — единый, в `performLayout` (через `clampWidthTo`): не меньше `MIN_LEFT_WIDTH`
(влезает заголовок «  EXPLORER») и так, чтобы редактору осталось не меньше `MIN_CENTER_WIDTH`.
`performLayout` клэмпит **только отображаемую** ширину и не мутирует `leftPanelWidth` — поэтому
временно узкий терминал не «съедает» абсолютный размер навсегда. Интерактивные изменения
(перетаскивание, команды) клэмпят сохранённое значение сразу, чтобы кнопки не «залипали».

### SashElement — перетаскиваемый разделитель

`SashElement` (`src/TUIDom/Widgets/SashElement.ts`) — невидимый хит-таргет шириной 1 колонка
на границе панели и редактора. Он добавляется в `getChildren()` **последним**, чтобы
`elementFromPoint` находил его поверх редактора, и раскладывается в `performLayout` на колонке
`leftWidth`.
...
поймал бы «случайный» клик. Sash сообщает наверх абсолютный `screenX` границы, а
`WorkbenchLayoutElement` переводит его в ширину и клэмпит.
